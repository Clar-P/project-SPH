...
export default {
  ...
  computed: {
    ...
    // 将来提交订单最终选中的地址
    userDefaultAddress() {
      // return this.userAddress.filter((e) => e.isDefault == 1)
      // filter返回满足条件的元素组成的一个数组（返回了满足条件的对象的组成的数组，要多包了一层）
      // find 返回第一个满足条件的元素（这里直接返回了满足的对象）
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      //console.log(result);
      // 提交订单成功
      // 因为接口问题，mock的数据又不能返回200，所以用201代替一下
      if(result.code == 201){
        // 返回的不是200，所以拿不到data订单号，自己假装一个
        this.orderId = 145687
        // 路由跳转与路由传参
